...
- **AdaptiveCadenceController (cadence.py)** - Optional reflective cadence. Measures news intensity as a pre-dedup filtered message rate (messages/min) over a rolling window and maps it to an `IntensityLevel` (NORMAL/ELEVATED/SURGE) from the volume ratio against the rolling median baseline AND an absolute message-rate floor (crisis keywords were removed as a zero-precision signal); a Cloudflare Radar outage promotes one level. A level requires BOTH its ratio (e.g. 4x baseline for SURGE) and its absolute floor (`surge_floor_rate`/`elevated_floor_rate`, default 1.5 / 0.75 msg/min): the floor stops ordinary traffic (~0.4 msg/min), judged against a near-zero floored baseline, from clearing the 4x ratio and reading as a false surge — the bug that made the cadence flap all day. Each evaluation returns a frozen `CadenceDecision` (previous/new interval, level, `CadenceChangeReason`): escalation is immediate (down to `min_interval_minutes`), decay grows the interval by `decay_factor` per decay step, capped at `SUMMARY_INTERVAL_MINUTES` or `max_interval_minutes`. `decay_factor` defaults to 2.0 so a surge unwinds in a few meaningful steps (30→60→120→240→360) rather than a dozen ~25% micro-steps (30→38→47…) that left the interval pinned near the floor. Decay is deliberately slow: EACH one-step decay requires `calm_streak_runs` consecutive NORMAL full-runs, after which the streak resets so the next step must re-earn the same sustained calm. A single quiet window mid-surge holds the tighter interval instead of relaxing, and any escalation (full-run or probe) resets the streak. This prevents the decay/re-escalate flapping that otherwise posts contradictory "calming"/"surging" notices minutes apart during a bursty event. The escalate-only probe guards on the **interval** (it only ever tightens, never widens): because hysteresis can hold a tight interval at a calm level, guarding on level severity instead would let a rising-but-still-below level (e.g. ELEVATED's slower target while the interval is held tight) wrongly relax the cadence. Only a genuine surge ONSET (an escalation out of a calm NORMAL state, `CadenceDecision.is_surge_onset`) posts a one-line Persian notice via `post_alert` stating why the cadence tightened (news volume rise, internet outage) and the old-to-new interval. Decay (`CALM_DECAY`) and mid-event re-escalation (when the previous level was already ELEVATED/SURGE) reschedule the interval silently, so the channel is never spammed with contradictory "calming"/"rising" notices. State persists to `.cadence_state`. Gated behind `adaptive_cadence.enabled`; an optional escalate-only probe (`fast_escalation`) tightens cadence between runs. The probe's measurement window (`probe_window_minutes`, default 15) is decoupled from how often it runs (`probe_interval_minutes`, default 5): it runs every 5 min but averages its rate over the last 15 min, so a single bursty message cluster is not misread as a surge against the (long-window) full-run baseline.
